...
class AccesoDatos
{
    ...
    public function actualizarIncrementos($lista)
    {
        
        $stmt = $this->dbh->prepare("UPDATE Usuarios SET saldo=saldo*1.1 where Nombre = ?");
        // Devuelvo una tabla de usuarios
    ...
    public function actualizarBloqueos($lista) {
        $stmt1 = $this->dbh->prepare("UPDATE Usuarios SET bloqueo=0 where bloqueo=1");
        $stmt2 = $this->dbh->prepare("UPDATE Usuarios SET bloqueo=1 where Nombre = ?");
